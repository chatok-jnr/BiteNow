...
exports.loginCustomer = async(req, res) => {
  try{
    ...
    const customer = await Customer.findOne({customer_email:customer_email})
    ...
    const isPasswordValid = await bcrypt.compare(req.body.customer_password, customer.customer_password);
    if(!isPasswordValid) {
      return res.status(400).json({
        status:'failed',
    ...
    if(customer_status !== 'Active') {
      return res.status(400).json({
        status:'failed',
        message:'Your account has been suspended. Please contact the support team to learn more'
      });
    }
...
exports.verifyRiderOtp = async(req, res) => {
  ...
  try{
    ...
    res.status(201).json({
      status:'success',
      message:'If all is ok Admin will send a email to your about the confirmation of you as a rider'
    });   
  } catch(err) {
    res.status(400).json({
      status:'failed',
...
exports.loginRider = async (req, res) => {
  try{
    ...
    const rider = await Rider.findOne({rider_email:rider_email})
    .select('+rider_password +rider_status');
    ...
    if(rider.rider_status !== 'Approved') {
      ...
      if(rider_status === 'Rejected') {
        ...
        msg = 'Your account has been suspended. Please Contact the support team to learn more'
      }
    ...
    const isPasswordValid = await bcrypt.compare(rider_password, rider.rider_password);
...
exports.createRestaurantOwner = async (req, res) => {
  try {
    ...
    const htmlTemlate = `
      ...
      <p>This code will expires in 5 minutes</p>
    `
    await sendEmail(req.body.restaurant_owner_email, "Verify Your Restaurant Owner Account", htmlTemlate);

    res.status(201).json({
      status: "success",
      message: "To Active your account Enter the otp code send to your email address",
...
exports.restaurantOwnerVerification = async(req, res) => {
  try{
    ...
    res.status(200).json({
      status:'success',
      message:'Your Account is verified. Now Please wait for the admin approval'
    });
  } catch(err) {
    res.status(400).json({
      status:'failed',
...
exports.loginRestaurantOwner = async(req, res) => {
  try{
    ...
    const isPasswordValid = await bcrypt.compare(restaurant_owner_password, pass);
    if(!isPasswordValid) {
      return res.status(400).json({
        status:'failed',
    ...
    if(sts !== 'Active') {
      ...
      if(sts === 'Suspended') msg = "Your account has been suspended. To learn more please contatct the support team"
      return res.status(400).json({
        status:'failed',
        message:msg
      });
    ...
    const ownerResponse = {
      ...
      restaurant_owner_is_verified:restaurantOwner.restaurant_owner_is_verified
    }
