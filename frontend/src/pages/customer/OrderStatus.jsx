...
import { ShoppingCart, Package, Clock, Search, CheckCircle, Truck, ChefHat, User, MapPin, Phone, Home as HomeIcon, LogOut } from 'lucide-react';
...
const OrderStatus = () => {
  ...
  const getStatusProgress = (status) => {
    const statusOrder = ['pending', 'look_rider', 'preparing', 'ready_for_pickup', 'out_for_delivery', 'delivered'];
    const statusLower = status?.toLowerCase();
  ...
  const transformOrder = (order) => {
    ...
    return {
      ...
      restaurantImage: order.restaurant_id?.restaurant_image?.url || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=400&q=80',
      items: order.items?.map(item => ({
        name: item.food_id?.food_name || item.food_name || item.name || 'Item',
        image: item.food_id?.food_image?.url || 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=400&q=80',
      ...
      estimatedTime: formatDate(order.estimated_delivery_time) || 'Calculating...',
    ...
    >
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className={`${getStatusColor(order.status)} text-white px-4 py-2 rounded-full flex items-center space-x-2 font-semibold`}>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {order.items.map((item, index) => (
              <span key={index} className="bg-[#DDEEDB] text-gray-700 px-3 py-1 rounded-full text-sm flex items-center gap-2">
                <img src={item.image} alt={item.name} className="w-6 h-6 rounded object-cover" />
                {item.name}
                <span className="text-[#67A177] font-semibold">${item.price.toFixed(2)}</span>
              </span>
            ))}
          </div>
        </div>
        ...
        {/* Confirmation Pin */}
          {isActive && order.confirmationPin && order.status?.toLowerCase() === 'out_for_delivery' && (
            <div className="mb-4 bg-[#67A177] p-4 rounded-xl">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white text-sm font-medium mb-1">Confirmation PIN</p>
                  <p className="text-white text-xs opacity-90">Share this PIN with your delivery rider</p>
                </div>
                <div className="bg-white px-6 py-3 rounded-lg">
                  <p className="text-3xl font-bold text-[#67A177] tracking-wider">{order.confirmationPin}</p>
                </div>
              </div>
            </div>
          )}
        ...
        {/* Rider Info */}
        {isActive && order.rider && order.status?.toLowerCase() === 'out_for_delivery' && (
          <div className="mb-4 bg-[#DDEEDB] p-4 rounded-xl">
            <p className="text-sm font-semibold text-gray-700 mb-3">Your Delivery Rider</p>
            <div className="flex items-center justify-between">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          {isActive ? (
            ...
            <div className="bg-[#DDEEDB] p-3 rounded-lg">
              ...
              <p className="font-semibold text-gray-800">{order.deliveredTime}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-[#8DBC96]/30">
          <div>
            ...
            <p className="text-2xl font-bold text-[#67A177]">${order.total.toFixed(2)}</p>
          </div>
          {isActive ? (
  ...
  return (
    <div className="min-h-screen bg-[#C4E2C4]">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 relative animate-fadeIn">
            ...
            <h2 className="text-2xl font-bold mb-2 text-[#67A177]">Order #{selectedOrder.id}</h2>
            <h3 className="text-lg font-semibold mb-4 text-gray-800">{selectedOrder.restaurant}</h3>
            <div className="mb-4 flex items-center space-x-3">
              <img src={selectedOrder.restaurantImage} alt={selectedOrder.restaurant} className="w-16 h-16 rounded-xl object-cover" />
              <span className="text-gray-700 font-medium">{selectedOrder.restaurant}</span>
            </div>
            <div>
              <p className="text-sm text-gray-600 mb-2">Food Items Ordered:</p>
              <ul className="space-y-3">
                {selectedOrder.items.map((item, idx) => (
                  <li key={idx} className="flex items-center space-x-3 bg-[#DDEEDB] px-4 py-2 rounded-lg">
            ...
            <div className="mt-6">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-[#67A177] font-semibold">${selectedOrder.deliveryCharge ? selectedOrder.deliveryCharge.toFixed(2) : '0.00'}</span>
              </div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-gray-600">Total Amount</span>
                <span className="text-[#67A177] font-bold">${selectedOrder.total.toFixed(2)}</span>
              </div>
              {selectedOrder.confirmationPin && (
                <div className="flex justify-between items-center mt-4 bg-[#67A177] px-4 py-2 rounded-lg">
                  <span className="text-white font-medium">Confirmation PIN</span>
                  <span className="bg-white text-[#67A177] font-bold px-4 py-2 rounded-lg text-xl tracking-wider">{selectedOrder.confirmationPin}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-[#8DBC96] py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-3">My Orders</h1>
          <p className="text-white/90 text-lg">Track and manage your food deliveries</p>
        </div>
      </div>
              ...
              {activeOrders.length > 0 ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {activeOrders.map((order) => (
                    <OrderCard key={order._id || order.id} order={transformOrder(order)} isActive={true} />
                  ))}
                </div>
              ) : (
                <div className="text-center py-16">
              ...
              {completedOrders.length > 0 ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {completedOrders.map((order) => (
                    <OrderCard key={order._id || order.id} order={transformOrder(order)} isActive={false} />
                  ))}
                </div>
              ) : (
                <div className="text-center py-16">
