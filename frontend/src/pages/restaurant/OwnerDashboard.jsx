...
function OwnerDashboard() {
  ...
  const fetchOwnerProfile = async (ownerId) => {
    try {
      ...
      if (data.status === "success" && data.data?.restaurantOwner) {
        setOwnerProfile(data.data.restaurantOwner);
      }
    } catch (error) {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      if (userData) {
        const parsedUser = JSON.parse(userData);
        // Refetch owner profile to get updated data
        await fetchOwnerProfile(parsedUser.id);
        await fetchRestaurants(parsedUser.id);
      }
  ...
  const getTotalStats = () => {
    const totalRevenue = restaurants.reduce(
      (sum, r) => sum + r.restaurant_total_revenue,
      0
    );
    const totalSales = restaurants.reduce(
      (sum, r) => sum + r.restaurant_total_sales,
      0
    );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              {/* Owner Profile Image - Clickable */}
              <button
                onClick={() => setShowEditProfile(true)}
                className="w-12 h-12 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center flex-shrink-0 hover:ring-2 hover:ring-primary transition-all cursor-pointer"
                title="View Profile"
              >
                {ownerProfile?.restaurant_owner_image?.url ? (
                  <img
                    src={ownerProfile.restaurant_owner_image.url}
                    alt={ownerProfile?.restaurant_owner_name}
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Welcome back,{" "}
                  {ownerProfile?.restaurant_owner_gender === "Male"
                    ? "Mr."
                    : ownerProfile?.restaurant_owner_gender === "Female"
                    ? "Ms."
                    : ""}{" "}
                  {ownerProfile?.restaurant_owner_name || "..."}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-primary border border-primary rounded-lg hover:bg-red-600 transition-colors"
              >
                Logout
              </button>
            </div>
                ...
                <div className="flex gap-6 min-w-max">
                  {restaurants.map((restaurant) => (
                    <div
                      ...
                      onClick={() =>
                        navigate(`/restaurant-manager/${restaurant._id}`)
                      }
                    >
                      {/* Restaurant Image */}
                      <div className="relative h-48 overflow-hidden rounded-t-lg">
                        ...
                        />
                        <span
                          className={`absolute top-3 right-3 px-3 py-1 text-xs font-medium rounded-full backdrop-blur-sm ${
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                          {restaurant.restaurant_category?.map(
                            (category, idx) => (
                              <span
                                key={idx}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              ...
              <strong>{ownerProfile?.restaurant_owner_email}</strong> to verify
        ...
        >
          ...
          >
            ...
            <ul className="list-disc list-inside text-sm text-gray-600 mb-6 space-y-1">
              <li>
                Delete all your restaurants ({restaurants.length}{" "}
