...
const Manage_Restaurant = () => {
  ...
  const handleAddFood = async () => {
    ...
    try {
      ...
      const foodData = {
        ...
        tags: foodForm.tags ? foodForm.tags.split(',').map(tag => tag.trim()) : []
      };
      ...
      if (imageFile && newFood._id) {
        try {
          ...
          setSuccess('Food added but image upload failed. You can add an image later.');
        }
      }
  ...
  const filteredOrders = categorizedOrders[orderStatus]?.filter(o => 
    o.customer_id?.customer_name?.toLowerCase().includes(searchQuery.toLowerCase()) || 
  ...
  const OrderCard = ({ order }) => {
    ...
    return (
      <div className="bg-[#ACD4B1] rounded-2xl p-6 shadow-lg cursor-pointer hover:shadow-xl transition-shadow" 
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={`px-3 py-1 rounded-full text-sm font-semibold text-white ${statusColors[order.order_status]}`}>
        ...
        <div className="bg-[#DDEEDB] p-4 rounded-xl mb-4">
          ...
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm">{order.customer_id?.customer_name || 'N/A'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#DDEEDB] p-4 rounded-xl mb-4">
          ...
          <div className="flex justify-between text-lg pt-2 border-t border-[#8DBC96]/30">
            ...
            <span className="font-bold text-[#67A177]">${total.toFixed(2)}</span>
          </div>
        </div>
        ...
        {order.order_status === 'pending' && (
          <div className="grid grid-cols-2 gap-3">
            <button 
              onClick={(e) => { e.stopPropagation(); handleUpdateOrderStatus(order._id, 'cancelled'); }} 
              className="bg-red-500 text-white py-2 rounded-full hover:bg-red-600 font-semibold"
            >
              Reject
            </button>
            <button 
              onClick={(e) => { e.stopPropagation(); handleUpdateOrderStatus(order._id, 'look_rider'); }} 
        ...
        {order.order_status === 'look_rider' && (
          <button 
            onClick={(e) => { e.stopPropagation(); handleUpdateOrderStatus(order._id, 'preparing'); }} 
        ...
        {order.order_status === 'preparing' && (
          <button 
            onClick={(e) => { e.stopPropagation(); handleUpdateOrderStatus(order._id, 'ready_for_pickup'); }} 
            className="w-full bg-[#67A177] text-white py-3 rounded-full hover:bg-[#5a8f68] font-semibold flex items-center justify-center space-x-2"
          >
        ...
        {order.order_status === 'ready_for_pickup' && (
          <button 
            onClick={(e) => { e.stopPropagation(); setSelectedOrder(order); setShowPinModal(true); }} 
            className="w-full bg-[#67A177] text-white py-3 rounded-full hover:bg-[#5a8f68] font-semibold flex items-center justify-center space-x-2"
          >
    ...
    return (
      <div className="min-h-screen bg-[#C4E2C4] flex items-center justify-center">
        <div className="bg-white rounded-2xl p-8 shadow-xl text-center max-w-md">
          ...
          <p className="text-gray-600">Please create a restaurant first or log in as a restaurant owner.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-[#8DBC96] py-8 shadow-md">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center space-x-6">
            ...
            <div>
              <h1 className="text-4xl font-bold text-white mb-2">Manage Restaurant</h1>
          ...
          <div>
            ...
            {!loading && filteredFoods.length === 0 && (
              <div className="text-center py-16">
                ...
                <h3 className="text-2xl font-bold text-gray-600">No Food Items</h3>
                <p className="text-gray-500 mt-2">Add your first food item to get started</p>
              </div>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredFoods.map(food => (
                <div key={food._id} className="bg-[#ACD4B1] rounded-2xl overflow-hidden shadow-lg relative">
                  <div className="absolute top-4 right-4 z-10">
                    <button onClick={() => setActiveDropdown(activeDropdown === food._id ? null : food._id)} className="bg-white p-2 rounded-full shadow-lg"><MoreVertical className="w-5 h-5" /></button>
                    {activeDropdown === food._id && (
                      <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl overflow-hidden">
                        <button onClick={() => openEditModal(food)} className="w-full px-4 py-3 text-left hover:bg-[#DDEEDB] flex items-center space-x-2"><Edit2 className="w-4 h-4" /><span>Update</span></button>
                        <button onClick={() => handleRestock(food._id)} className="w-full px-4 py-3 text-left hover:bg-[#DDEEDB] flex items-center space-x-2"><Package className="w-4 h-4" /><span>Restock</span></button>
                        <button onClick={() => handleDeleteFood(food._id)} className="w-full px-4 py-3 text-left hover:bg-red-50 flex items-center space-x-2 text-red-600"><Trash2 className="w-4 h-4" /><span>Delete</span></button>
                      </div>
                    )}
                  </div>
                  <div className="aspect-square overflow-hidden cursor-pointer bg-gray-200 flex items-center justify-center" onClick={() => { setSelectedFood(food); setShowFoodDetailsModal(true); }}>
                    {food.food_image?.url ? (
                      <img src={food.food_image.url} alt={food.food_name} className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                    ) : (
                  ...
                  <div className="p-5">
                    ...
                    <div className="mb-3">
                      {food.discount_percentage > 0 ? (
                        <div className="flex items-center space-x-2">
                          <span className="text-2xl font-bold text-[#67A177]">${(food.food_price * (1 - food.discount_percentage / 100)).toFixed(2)}</span>
                          <span className="text-sm text-gray-500 line-through">${food.food_price.toFixed(2)}</span>
                          <span className="bg-red-500 text-white px-2 py-1 rounded-full text-xs font-semibold">{food.discount_percentage}% OFF</span>
                        </div>
                      ) : (
                        <span className="text-2xl font-bold text-[#67A177]">${food.food_price.toFixed(2)}</span>
                      )}
                    </div>
                    <div className="bg-[#DDEEDB] px-3 py-2 rounded-lg">
                      <p className="text-sm text-gray-600">Stock: <span className={`font-bold ${food.food_quantity < 10 ? 'text-red-600' : 'text-gray-800'}`}>{food.food_quantity}</span></p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div>
            <div className="mb-6">
              <div className="relative max-w-md mb-4">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search orders..." className="w-full pl-10 pr-4 py-3 rounded-full border-2 border-[#8DBC96] focus:border-[#67A177] focus:outline-none bg-white" />
              </div>
              
              <div className="flex flex-wrap gap-2">
                {['pending', 'look_rider', 'preparing', 'ready_for_pickup', 'out_for_delivery', 'delivered', 'cancelled'].map(status => (
                  <button 
                    key={status} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {!loading && filteredOrders.length === 0 && (
                <div className="col-span-3 text-center py-16">
                  <Package className="w-20 h-20 text-gray-400 mx-auto mb-4" />
                  <h3 className="text-2xl font-bold text-gray-600">No Orders</h3>
                  <p className="text-gray-500 mt-2">No {orderStatus} orders at the moment</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-[#DDEEDB] rounded-2xl max-w-2xl w-full p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setShowAddFoodModal(false); setFoodForm({ food_name: '', food_description: '', food_price: '', food_quantity: '', discount_percentage: 0, tags: '' }); setImageFile(null); setImagePreview(null); }}><X className="w-6 h-6" /></button>
            </div>
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-4">
                  <input type="file" accept="image/*" onChange={handleFoodImageUpload} className="w-full px-4 py-3 rounded-lg border-2 border-[#8DBC96] focus:border-[#67A177] focus:outline-none bg-white" />
                  {imagePreview && (
                    <img src={imagePreview} alt="Preview" className="w-20 h-20 object-cover rounded-lg border-2 border-[#8DBC96]" />
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-[#DDEEDB] rounded-2xl max-w-2xl w-full p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setShowFoodDetailsModal(false); setSelectedFood(null); }}><X className="w-6 h-6" /></button>
            </div>
            <img src={selectedFood.image} alt={selectedFood.name} className="w-full h-64 object-cover rounded-xl mb-4" />
            <div className="space-y-3">
              <div className="bg-[#ACD4B1] p-4 rounded-xl"><p className="text-sm text-gray-600">Price</p><p className="text-2xl font-bold text-[#67A177]">${selectedFood.price.toFixed(2)}</p></div>
              <div className="bg-[#ACD4B1] p-4 rounded-xl"><p className="text-sm text-gray-600">Stock</p><p className="text-xl font-bold">{selectedFood.quantity} units</p></div>
              {selectedFood.discount > 0 && <div className="bg-[#ACD4B1] p-4 rounded-xl"><p className="text-sm text-gray-600">Discount</p><p className="text-xl font-bold text-red-600">{selectedFood.discount}%</p></div>}
              <div className="bg-[#ACD4B1] p-4 rounded-xl"><p className="text-sm text-gray-600 mb-2">Description</p><p className="text-gray-800">{selectedFood.description || 'No description'}</p></div>
            </div>
          </div>
        </div>
      )}
      ...
      {showPinModal && selectedOrder && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-[#DDEEDB] rounded-2xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-4">Enter the PIN from the rider to confirm pickup</p>
            <input type="text" value={riderPin} onChange={(e) => setRiderPin(e.target.value)} placeholder="Enter 4-digit PIN" className="w-full px-4 py-3 rounded-lg border-2 border-[#8DBC96] focus:border-[#67A177] focus:outline-none bg-white mb-4 text-center text-2xl font-bold tracking-widest" maxLength="4" />
            <div className="flex space-x-4">
              <button onClick={() => { setShowPinModal(false); setRiderPin(''); }} className="flex-1 bg-gray-400 text-white py-3 rounded-full hover:bg-gray-500 font-semibold">Cancel</button>
              <button onClick={handleVerifyPin} className="flex-1 bg-[#67A177] text-white py-3 rounded-full hover:bg-[#5a8f68] font-semibold">Verify</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-[#DDEEDB] rounded-2xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setShowOrderDetailsModal(false); setSelectedOrder(null); }}><X className="w-6 h-6" /></button>
            </div>
            <div className="space-y-4">
              <div className="bg-[#ACD4B1] p-4 rounded-xl">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className={`px-3 py-1 rounded-full text-sm font-semibold text-white ${
                    ...
                    selectedOrder.order_status === 'look_rider' ? 'bg-orange-500' : 
                    selectedOrder.order_status === 'ready_for_pickup' ? 'bg-purple-500' : 
                    selectedOrder.order_status === 'preparing' ? 'bg-blue-500' : 
                    selectedOrder.order_status === 'out_for_delivery' ? 'bg-green-500' : 
                    selectedOrder.order_status === 'delivered' ? 'bg-[#67A177]' : 
                    'bg-gray-500'
                  }`}>
                    {selectedOrder.order_status ? 
                      selectedOrder.order_status.replace(/_/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') 
                      : 'Unknown'
                    }
                  </div>
                </div>
              ...
              <div className="bg-[#ACD4B1] p-4 rounded-xl">
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2"><User className="w-5 h-5 text-[#67A177]" /><span className="font-semibold">{selectedOrder.customer.name}</span></div>
                  <div className="flex items-center space-x-2"><Phone className="w-5 h-5 text-[#67A177]" /><span>{selectedOrder.customer.phone}</span></div>
                  <div className="flex items-center space-x-2"><Mail className="w-5 h-5 text-[#67A177]" /><span>{selectedOrder.customer.email}</span></div>
                  <div className="flex items-start space-x-2"><MapPin className="w-5 h-5 text-[#67A177] mt-0.5" /><span>{selectedOrder.customer.address}</span></div>
                  {selectedOrder.customer.message && <div className="flex items-start space-x-2"><MessageSquare className="w-5 h-5 text-[#67A177] mt-0.5" /><span className="italic">{selectedOrder.customer.message}</span></div>}
                </div>
              </div>
              <div className="bg-[#ACD4B1] p-4 rounded-xl">
                ...
                <div className="space-y-2">
                  {selectedOrder.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center py-2 border-b border-[#8DBC96]/30 last:border-0">
                      <div>
                        <p className="font-semibold">{item.food}</p>
                        <p className="text-sm text-gray-600">Quantity: {item.quantity}</p>
                      </div>
                      <p className="font-bold text-[#67A177]">${(item.price * item.quantity).toFixed(2)}</p>
                    </div>
                  ))}
                </div>
              </div>
              {selectedOrder.rider && (
                <div className="bg-[#67A177] p-4 rounded-xl text-white">
                  <p className="text-sm font-semibold mb-3">Rider Information</p>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-lg">{selectedOrder.rider.name}</p>
                  ...
                  {selectedOrder.confirmationPin && (
                    <div className="mt-3 pt-3 border-t border-white/20">
                      ...
                      <p className="text-3xl font-bold">{selectedOrder.confirmationPin}</p>
                    </div>
                  )}
                </div>
              )}
              <div className="bg-[#ACD4B1] p-4 rounded-xl">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between"><span>Food Total</span><span className="font-semibold">${calculateTotal(selectedOrder.items).toFixed(2)}</span></div>
                  <div className="flex justify-between"><span>Delivery Cost</span><span className="font-semibold">${selectedOrder.deliveryCost.toFixed(2)}</span></div>
                  <div className="flex justify-between text-xl pt-2 border-t border-[#8DBC96]/30"><span className="font-bold">Total</span><span className="font-bold text-[#67A177]">${(calculateTotal(selectedOrder.items) + selectedOrder.deliveryCost).toFixed(2)}</span></div>
                </div>
              </div>
            </div>
          </div>
